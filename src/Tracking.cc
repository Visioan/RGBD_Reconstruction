...
namespace SIFT_SLAM
{
	...
	Tracking::Tracking(System *pSys, SIFTVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Map *pMap, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor) :
		mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false), mbVO(false), mpORBVocabulary(pVoc),
		mpKeyFrameDB(pKFDB), mpInitializer(static_cast<Initializer*>(NULL)), mpSystem(pSys), mpViewer(NULL),
		mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0), mbisLost(false), testid(0)
	{
		mplasttracking = nullptr;
		mpreindex = -1;
		...
		//如果默认阈值提取不出最够的FAST特征点，则使用最小阈值 8
		int fMinThFAST = fSettings["ORBextractor.minThFAST"];

		//Tracking过程中，都会用到mpORBextractorLeft作为特征点提取器
		char* myargv[3] = { "-v", "0", "-cuda" };
		//mpSIFTextractor = new Featureextractor(nFeatures, fScaleFactor, nLevels);
#ifdef USE_GPU_SIFT
		mpSIFTextractor = new Featureextractor(3, myargv);
#else
		mpSIFTextractor = new Featureextractor(nFeatures, fScaleFactor, nLevels);
#endif

		std::cout << endl << "SIFT Extractor Parameters: " << endl;
		...
		mIniPose = cv::Mat::eye(4, 4, CV_32F);
	...
	cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD, const double &timestamp,int i,int total)
	{
		mImGray = imRGB;
		//cv::Mat imDepth = imD;
		mImRGB = imRGB;
		...
		// 步骤3：构造Frame
		mCurrentFrame = Frame(mImGray, mImDepth, mtimestamp, mpSIFTextractor, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth);
		// 步骤4：跟踪
		Track(i,total);
	...
	void Tracking::Track(int iframe,int totalframe)
	{
		...
		else// 步骤2：跟踪
		{
			...
			if (bOK)
				bOK = TrackLocalMap();
			//if (!bOK)
			//	bOK = InsertLastFrameAsKey(mLastFrame);
			...
			if (bOK)
			{
				...
				if (NeedNewKeyFrame())
				{
					std::cout << "CreateNewKeyFrame  " << mCurrentFrame.mnId << endl;
					CreateNewKeyFrame();
					//bool isok = InsertLastFrameAsKey(mLastFrame);
					mpreKeymatch = mcurKeymatch;
			...
			if (mState == LOST)
			{
				mlastTcw = mLastFrame.mTcw;
				mCurrentFrame.SetPose(mlastTcw);
				bool isok=InsertLastFrameAsKey(mLastFrame);
				//InsertFrameAsKey(mCurrentFrame);
				//InsertFrameAsKey(mLastFrame);
				if (isok)
					mState = OK;
				else
					mState = LOST;
			...
			if (!mCurrentFrame.mpReferenceKF)
				mCurrentFrame.mpReferenceKF = mpReferenceKF;
					// 保存上一帧的数据
			//	mCurrentFrame.mplasttracking = this;
			mplasttracking = this;
		...
		if (bcreatekf || binsertkf)//最后一帧或者在插入关键帧时候进行了BA优化
		{
			bcreatekf = false;//设置标志位
			Frame tempLastFrame = Frame(mvFrame.back());
			if (binsertkf&&mvFrame.size()>1)
			{
				mvFrame.pop_back();
				fmvFramemat.pop_back();
				mvFramemat.pop_back();
			}

			mvKeyFramePosesNBA.push_back(mvFrame[0].mTcw);

			cv::Mat Rcw = mvFrame[0].mTcw.rowRange(0, 3).colRange(0, 3);
			cv::Mat tcw = mvFrame[0].mTcw.rowRange(0, 3).col(3);
			cv::Mat Rwc = Rcw.t();
			cv::Mat Ow = -Rwc*tcw;

			cv::Mat Twc = cv::Mat::eye(4, 4, mvFrame[0].mTcw.type());
			Rwc.copyTo(Twc.rowRange(0, 3).colRange(0, 3));
			Ow.copyTo(Twc.rowRange(0, 3).col(3));

			for (size_t i = 0; i < mvFrame.size(); i++)
			{

				cv::Mat Tcrf = fmvFramemat[i] * Twc;
				cv::Mat Tcr = mvFramemat[i] * Twc;
				mlRelativeFramePosesBA.push_back(Tcrf);//没有经过BA优化的局部姿态
				mlRelativeKeyFramePosesNBA.push_back(mvFrame[0].mTcw);//保存相对关键帧姿态
				mlRelativeFramePosesNBA.push_back(Tcr);
			}
			fmvFramemat.clear();
			mvFramemat.clear();
			mvFrame.clear();
			if (binsertkf)
				mvFrame.push_back(tempLastFrame);
			binsertkf = false;
		}
		if (!mCurrentFrame.mTcw.empty())
		{
			// 计算相对姿态T_currentFrame_referenceKeyFrame
			mvFrame.push_back(mCurrentFrame);
			...
			mlIndex.push_back(std::pair<int, std::vector<int>>(mpreindex, mCurrentFrame.mvindex));
			mvvpreKP.push_back(mprekp);
			mvvcurkp.push_back(mCurrentFrame.mvKeysUn);
			mlastTcw = mLastFrame.mTcw;
		}
		else
		{
		...
		if (iframe == totalframe - 1)//最后一帧
		{
			mvvFrame.push_back(mvFrame);
			std::vector<Frame> tempFrame = mvFrame;
			if (mvFrame.size() > 1)
			{
				Optimizer::LocalBundleAdjustment(mvFrame);
			}
			for (size_t i = 0; i < mvFrame.size(); i++)
			{
				fmvFramemat.push_back(mvFrame[i].mTcw.clone());
				mvFramemat.push_back(tempFrame[i].mTcw.clone());
			}

			fmvvFramemat.push_back(fmvFramemat);
			mvvFramemat.push_back(mvFramemat);

			mvKeyFramePosesNBA.push_back(mvFrame[0].mTcw);
			for (size_t i = 0; i < mvFrame.size(); i++)
			{

				cv::Mat Tcrf = fmvFramemat[i] * mvFrame[0].mTcw.inv();
				cv::Mat Tcr = mvFramemat[i] * mvFrame[0].mTcw.inv();
				mlRelativeFramePosesBA.push_back(Tcrf);//没有经过BA优化的局部姿态
				mlRelativeKeyFramePosesNBA.push_back(mvFrame[0].mTcw);//保存相对关键帧姿态
				mlRelativeFramePosesNBA.push_back(Tcr);
			}
	...
	*/
	...
	{
		std::cout << "StereoInitialization N= " << mCurrentFrame.N << std::endl;
		...
		//if (mCurrentFrame.N > 100)
		{
			...
			mCurrentFrame.SetPose(mIniPose.clone());
	...
	bool Tracking::InsertFrameAsKey(Frame &f)
	{
		while (mpLocalMapper->isStopped() || mpLocalMapper->stopRequested())
			cv::waitKey(10);
		cv::Mat temppose = f.mTcw;
		f = Frame(f.mImgray, f.mImdepth, f.mtimestamp, mpSIFTextractor, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth);
		f.ComputeBoW();
		if (f.N >= 100)
		{
			// Set Frame pose to the origin
			...
			// KeyFrame里有一个mpMap，Tracking里有一个mpMap，而KeyFrame里的mpMap都指向Tracking里的这个mpMap
			// KeyFrame里有一个mpKeyFrameDB，Tracking里有一个mpKeyFrameDB，而KeyFrame里的mpMap都指向Tracking里的这个mpKeyFrameDB
			KeyFrame* pKFini = new KeyFrame(f, mpMap, mpKeyFrameDB);
			// Insert KeyFrame in the map
			// KeyFrame中包含了地图、反过来地图中也包含了KeyFrame，相互包含
			// 步骤3：在地图中添加该初始关键帧
			mpMap->AddKeyFrame(pKFini);
			...
			for (int i = 0; i < f.N; i++)
			{
				...
				{
					// 步骤4.1：通过反投影得到该特征点的3D坐标
					cv::Mat x3D = f.UnprojectStereo(i);
					// 步骤4.2：将3D点构造为MapPoint
					MapPoint* pNewMP = new MapPoint(x3D, pKFini, mpMap);
					// 步骤4.3：为该MapPoint添加属性：
					// a.观测到该MapPoint的关键帧
					...
					// 步骤4.4：在地图中添加该MapPoint
					mpMap->AddMapPoint(pNewMP);
					f.mvpMapPoints[i] = pNewMP;
				}
			}
			std::cout << "InsertFrameAsKey New map created with " << mpMap->MapPointsInMap() << " points" << endl;
			// 步骤4：在局部地图中添加该初始关键帧
			mpLocalMapper->InsertKeyFrame(pKFini);
			//mLastFrame = Frame(mLastFrame);
			mnLastKeyFrameId = f.mnId;
			mpLastKeyFrame = pKFini;
			mvpLocalKeyFrames.push_back(pKFini);
			mvpLocalMapPoints = mpMap->GetAllMapPoints();
			mpReferenceKF = pKFini;
			f.mpReferenceKF = pKFini;
			// 把当前（最新的）局部MapPoints作为ReferenceMapPoints
			// ReferenceMapPoints是DrawMapPoints函数画图的时候用的
			mpMap->SetReferenceMapPoints(mvpLocalMapPoints);
			mpMap->mvpKeyFrameOrigins.push_back(pKFini);
			mpMapDrawer->SetCurrentCameraPose(f.mTcw);
			return true;
		}
		else
			return false;
	...
	bool Tracking::InsertLastFrameAsKey(Frame &f)
	{
		bcreatekf = true;
		binsertkf = true;
		//局部优化姿态
		...
		while (!mpLocalMapper->SetNotStop(true))
			cv::waitKey(10);
		// 步骤1：将当前帧构造成关键帧
		KeyFrame* pKF = new KeyFrame(f, mpMap, mpKeyFrameDB);
		...
		//if (mSensor != System::MONOCULAR)
		{
			...
			if (!vDepthIdx.empty())
			{
				...
				for (size_t j = 0; j < vDepthIdx.size(); j++)
				{
					...
					{
						bCreateNew = true;
						f.mvpMapPoints[i] = static_cast<MapPoint*>(NULL);
					}
				...
				std::cout << "InsertLastFrameAsKey  "<<f.mnId <<"  New map created with " << nPoints << " points" << endl;
			}
		...
		mpLastKeyFrame = pKF;
		//重新对当前帧跟踪
		cv::Mat Tcr = f.mTcw*f.mpReferenceKF->GetPoseInverse();
		//mlRelativeFramePoses.push_back(Tcr);
		mvKeyFramePosesNBA.back() = mpReferenceKF->GetPose().clone();
		mlRelativeKeyFramePosesNBA.back() = mpReferenceKF->GetPose().clone();
		mlRelativeFramePoses.back() = Tcr.clone();
		mlpReferences.back() = mpReferenceKF;
		//bool isbOK = TrackWithLastFrame();
		//// TrackReferenceKeyFrame是跟踪参考帧，不能根据固定运动速度模型预测当前帧的位姿态，通过bow加速匹配（SearchByBow）
		//// 最后通过优化得到优化后的位姿
		//if (!isbOK)
			// TrackReferenceKeyFrame是跟踪参考帧，不能根据固定运动速度模型预测当前帧的位姿态，通过bow加速匹配（SearchByBow）
		...
		bool isbOK = TrackReferenceKeyFrame();
		//mvFrame.push_back(mLastFrame);
		return isbOK;
	}
	/**
	...
	*/
	bool Tracking::TrackReferenceKeyFrame()
	{
		...
		mpreindex = mpReferenceKF->mnFrameId;
		mprekp = mpReferenceKF->mvKeysUn;
		mCurrentFrame.mvindex = std::vector<int>(mCurrentFrame.N, -1);
		// We perform first an ORB matching with the reference keyframe
		// If enough matches are found we setup a PnP solver
		SIFTmatcher matcher(0.85, true);
		...
		int nopmatch = Optimizer::PoseOptimization(&mCurrentFrame);
		// Discard outliers
	...
	bool Tracking::TrackReferenceKeyFrameBA()
	{
		...
		// 步骤2：通过特征点的BoW加快当前帧与参考帧之间的特征点匹配
		// 特征点的匹配关系由MapPoints进行维护
		int nmatches = matcher.SearchByBoWKeyPoints(*mpReferenceKF, mCurrentFrame, vindex);
		...
		// 步骤4:通过优化3D-2D的重投影误差来获得位姿
		int nopmatch = Optimizer::PoseOptimizationBA(*mpReferenceKF, mCurrentFrame, vindex);
		// Discard outliers
		// 步骤5：剔除优化后的outlier匹配点（MapPoints）
		mCurrentFrame.mvpMapPoints = vector<MapPoint*>(mCurrentFrame.N, static_cast<MapPoint*>(NULL));
		vector<MapPoint*> vpMapPointRef = mpReferenceKF->GetMapPointMatches();
		int nmatchesMap = 0;
		for (int i = 0; i < vindex.size(); i++)
		{
			...
			{
				mCurrentFrame.mvpMapPoints[i] = vpMapPointRef[vindex[i]];
				nmatchesMap++;
			}
		}
		mmatch = nmatchesMap;
	...
	*/
	bool Tracking::TrackWithMotionModel()
	{
		...
		mpreindex = mLastFrame.mnId;
		mprekp = mLastFrame.mvKeysUn;
		mCurrentFrame.mvindex = std::vector<int>(mCurrentFrame.N, -1);
		...
		//int nmatches = matcher.SearchByBoW(mCurrentFrame, mLastFrame);
		if (nmatches < 10)
			return false;
	...
	*/
	bool Tracking::TrackLocalMap()
	{
		...
		// 步骤1：更新局部关键帧mvpLocalKeyFrames和局部地图点mvpLocalMapPointss
		UpdateLocalMap();
		...
		int nnum = Optimizer::PoseOptimization(&mCurrentFrame);
		mnMatchesInliers = 0;
	...
	*/
	...
	//	//const bool c1a = mcurKeymatch<0.3*mpreKeymatch;
	...
	//	const bool c2 = ((mnMatchesInliers < nRefMatches*thRefRatio || bNeedToInsertClose) && mnMatchesInliers<50);
	//	if (c1a || c1b || c1c && c2)
	//	{
	...
	*/
	...
	void Tracking::CreateNewKeyFrame()
	{
		...
		//mpPointCloudMapping->insertKeyFramePointCloud(pKF, this->mImRGB, this->mImDepth);
	...
	//		cv::waitKey(10);
	//	// 步骤1：将当前帧构造成关键帧
	//	KeyFrame* pKF = new KeyFrame(mLastFrame, mpMap, mpKeyFrameDB);
	//	// 步骤2：将当前关键帧设置为当前帧的参考关键帧
	//	// 在UpdateLocalKeyFrames函数中会将与当前关键帧共视程度最高的关键帧设定为当前帧的参考关键帧
	//	mpReferenceKF = pKF;
	//	mCurrentFrame.mpReferenceKF = pKF;
	//	mLastFrame.mpReferenceKF = pKF;
	//	// 这段代码和UpdateLastFrame中的那一部分代码功能相同
	//	// 步骤3：对于双目或rgbd摄像头，为当前帧生成新的MapPoints
	//	if (mSensor != System::MONOCULAR)
	//	{
	//		// 根据Tcw计算mRcw、mtcw和mRwc、mOw
	//		mLastFrame.UpdatePoseMatrices();
	//
	//		// We sort points by the measured depth by the stereo/RGBD sensor.
	//		// We create all those MapPoints whose depth < mThDepth.
	//		// If there are less than 100 close points we create the 100 closest.
	...
	*/
	void Tracking::SearchLocalPoints()
	{
		...
		{
			MapPoint* pMP = *vit;
			// 已经被当前帧观测到MapPoint不再判断是否能被当前帧观测到s
	...
	*/
	void Tracking::UpdateLocalKeyFrames()
	{
		...
		{
			//for (vector<KeyFrame*>::const_iterator itKF = mvpLocalKeyFrames.begin(), itEndKF = mvpLocalKeyFrames.end(); itKF != itEndKF; itKF++)
			for (int itKF = 0; itKF <mvpLocalKeyFrames.size(); itKF++)
			{
		...
		// 步骤3：更新当前帧的参考关键帧，与自己共视程度最高的关键帧作为参考关键帧s
		if (pKFmax)
		{
	...
	bool Tracking::Relocalization()
	{
		...
		while (nCandidates > 0 && !bMatch)
		{
			for (int i = 0; i < nKFs; i++)
			{
				if (vbDiscarded[i])
					continue;
				mpreindex = vpCandidateKFs[i]->mnFrameId;
				mprekp = vpCandidateKFs[i]->mvKeysUn;
